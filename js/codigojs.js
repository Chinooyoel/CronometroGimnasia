window.onload = () => {
  ...
  document.getElementById("nuevoEjercicio").addEventListener("click", () => {
    if (activoTemporizador == false) {
      nuevoEjercicio();
      actualizarIndiceDeCantidadDeNodos(
        document.querySelectorAll("#contenedor h2"),
        "Ejercicio"
      );
      numeroDeSerie = 1;
      hayEjercicio = true;
  ...
  document
    .getElementById("restablecerCronometro")
...
function modificarCronometroHTML(reloj) {
  modificarTextoDeUnNodo(
    agregarCero(reloj.segundo),
    document.getElementById("segundo")
  );
  modificarTextoDeUnNodo(
    agregarCero(reloj.minuto),
    document.getElementById("minuto")
  );
  modificarTextoDeUnNodo(
    agregarCero(reloj.hora),
...
function nuevoEjercicio() {
  ...
  document
    ...
    .appendChild(nuevo_article)
    .appendChild(nuevo_h2);
  //Crear boton Eliminar
  document
    .getElementById("contenedor")
    .appendChild(nuevo_article)
    .appendChild(nuevo_button);
  //Crea el hijo lista
...
function nuevaSerie(objetoTiempo, numeroDeSerie) {
  ...
  document
    ...
    .lastChild.lastChild.appendChild(nuevo_li);

  modificarTextoDeUnNodo(
    `${numeroDeSerie}°Serie ${agregarCero(objetoTiempo.hora)}:${agregarCero(
      objetoTiempo.minuto
    )}:${agregarCero(objetoTiempo.segundo)}`,
    document.getElementById("contenedor").lastChild.lastChild.lastChild
  );

  document
    .querySelector("#contenedor")
    .lastChild.lastChild.lastChild.appendChild(nuevo_button);
}

function actualizarIndiceDeCantidadDeNodos(
  arraysDeHijosNodos,
  textoInformativo
) {
